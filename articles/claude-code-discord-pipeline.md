...
もともと、X(旧Twitter)で見つけた技術記事や「あとで読もう」と思った論文を、Discordに自分用のサーバーを立てて、ジャンル別チャンネルに投げ込んで管理していました。AI、プログラミング、デザイン、論文…とチャンネルを分けて、URLを貼っていく運用です。

ただ、貼るだけで満足してしまって、結局見返さない「あとで読む墓場」になっていました。

そんなときに **Claude CodeがDiscordプラグインを公開** しました。「DiscordとClaude Codeが繋がる」と聞いた瞬間、——自分が情報収集に使っているDiscordチャンネルと、整理用のGitHubリポジトリを繋げばうまくいけるのでは、と気づきました。

### 試行錯誤

最初はDiscordの **個人チャット(DM)** でClaude Codeとやり取りする形で試しました。シンプルで、すぐ動きそうだったので。

ただ実際使ってみると、AIも論文もプログラミングのURLも **全部DMに混ざる形** になり、かなり使いにくい状態でした。せっかくチャンネルで分けて整理してきたのに、それが活きない構成になってしまっていました。

そこで方針転換して、**Discordサーバーのチャンネル側にClaude Codeを呼ぶ** 形にしました。さらに「特定のスレッドだけ見る」設定にして、必要なときだけ反応するようにしています。

これで **チャンネルごとに整理しつつ、見たいスレッドだけClaude Codeが処理する** という現在の運用に落ち着きました。
